...
You must output a JSON object adhering to the following structure:
{
  ...
  // 7. Key Competency Deep Assessment (关键能力深度评估)
  ...
  // 9. Personality Assessment (性格测试) — Inferred from interview behavior
  "personalityAssessment": {
...
`;
    ...
    try {
      ...
      if (!(result as any).keyCompetencyAssessment) {
        (result as any).keyCompetencyAssessment = {
      ...
      if (!(result as any).skillRadar) {
        (result as any).skillRadar = {
