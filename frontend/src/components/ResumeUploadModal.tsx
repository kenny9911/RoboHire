...
export default function ResumeUploadModal({ open, onClose, onUploaded, batch = false }: ResumeUploadModalProps) {
  ...
  const handleFiles = (files: FileList | null) => {
    if (!files) return;
    const arr = Array.from(files).slice(0, batch ? 10 : 1);
  ...
  const handleUpload = async () => {
    ...
    try {
      if (batch && selectedFiles.length > 1) {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-900">
            {batch ? t('resumeLibrary.uploadModal.batchTitle', 'Batch Upload Resumes') : t('resumeLibrary.uploadModal.title', 'Upload Resume')}
          </h3>
        ...
        >
          ...
          <p className="text-sm text-gray-600 mb-1">{t('resumeLibrary.uploadModal.dragDrop', 'Drag & drop resume files here')}</p>
          <p className="text-xs text-gray-400">{t('resumeLibrary.uploadModal.formats', 'Supported: PDF, DOCX, TXT')}</p>
          {batch && <p className="text-xs text-gray-400 mt-1">{t('resumeLibrary.uploadModal.maxFiles', 'Up to 10 files at once')}</p>}
          <input
            ref={fileInputRef}
            type="file"
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          {!allDone && (
            ...
            >
              ...
              {uploading ? t('resumeLibrary.uploadModal.uploading', 'Processing...') : t('resumeLibrary.upload', 'Upload')}
            </button>
          )}
        </div>
      </div>
