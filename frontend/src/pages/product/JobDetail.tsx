...
interface Job {
  ...
  alexSession?: {
    id: string;
    title: string;
    messages: { id: string; role: string; text: string; isThinking?: boolean; isError?: boolean }[];
      ...
      {activeTab === 'details' && (
        <div className="flex gap-6">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            {/* Agent Alex Chat History — shown when job was created from Agent Alex */}
            {job.alexSession && (
              <div className="rounded-xl border border-indigo-200 bg-white overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3 border-b border-indigo-100 bg-indigo-50/50">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-indigo-900">
                      {t('product.jobDetail.alexChatHistory', 'Agent Alex — Conversation History')}
                    </h3>
                  </div>
                  <a
                    href="/agent-alex"
                  ...
                  >
                    {t('product.jobDetail.openAgentAlex', 'Open Agent Alex')}
                  </a>
                </div>
                <div className="px-5 py-4 max-h-[500px] overflow-y-auto space-y-3 custom-scrollbar">
                  {job.alexSession.messages
                    .filter(m => m.id !== 'welcome' && !m.isThinking && m.text)
                    .map((msg) => (
                      <div
                        key={msg.id}
                      ...
                      >
                        <div
                          className={`max-w-[85%] rounded-2xl text-sm leading-relaxed ${
                            msg.role === 'user'
                              ? 'px-3.5 py-2.5 bg-indigo-600 text-white rounded-br-sm'
                              : 'px-4 py-3 bg-slate-100 text-slate-700 rounded-bl-sm'
                          }`}
                        >
                          {msg.role === 'model' ? (
                            <div className="prose prose-sm max-w-none text-slate-700 [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 prose-p:my-2 prose-ul:my-2 prose-li:my-0.5 prose-strong:text-slate-900">
...
function BasicInfoField({ icon, iconBg, iconColor, label, value, editing, editValue, onEditChange, editType, selectOptions, className, allowWrap }: {
  ...
  return (
    <div className={`flex gap-3 ${allowWrap ? 'items-start' : 'items-center'} ${className || ''}`}>
      ...
      <div className="min-w-0 flex-1">
        ...
        {editing ? (
          ...
          <p className="text-sm font-semibold text-slate-800 whitespace-pre-wrap break-words leading-relaxed">{displayValue}</p>
        ) : (
